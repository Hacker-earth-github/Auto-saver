...
contract AutoSaver {
    ...
    function withdraw(uint256 _amount) external {
        require(
            _amount <= manualBalances[msg.sender],
            "Insufficient manual balance"
        );
    ...
    function deposit(uint256 _amount) external {
        ...
        if (
            autoDepositEnabledFor[msg.sender] &&
            block.timestamp >= endDepositDateFor[msg.sender]
        ) {
            require(
    ...
    function checkAutoActions() external {
        if (
            autoWithdrawEnabledFor[msg.sender] &&
            block.timestamp >= endWithdrawDateFor[msg.sender]
        ) {
            performAutoWithdraw();
        }
    ...
    function performAutoWithdraw() internal {
        require(
            withdrawalAmount <= automatedBalances[msg.sender],
            "Insufficient automated balance"
        );
